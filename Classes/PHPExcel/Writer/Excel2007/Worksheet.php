...
//      }
    ...
    private function writeAutoFilter(PHPExcel_Shared_XMLWriter $objWriter = null, PHPExcel_Worksheet $pSheet = null)
    {
        ...
        if (!empty($autoFilterRange)) {
            ...
            if (count((is_countable($columns)?$columns:[]))) { //if (count($columns > 0)) { //updated: add array to avoid throwing error
            ...
            if (is_object($cellValue) || $cellValue !== '') {
                ...
                switch (strtolower($mappedType)) {
                    ...
                    case 'f':            // Formula
                        ...
                        if ((is_string($attributes) && $attributes['t']) == 'array') { //if ($attributes['t'] == 'array') { //altering this original line to avoid 'null' error
